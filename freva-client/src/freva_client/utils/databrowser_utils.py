...
class Config:
    ...
    def search_url(self) -> str:
        ...
        This is the root URL that can be passed to fsspec or xarray to open
        a Zarr store streamed via HTTP. The `token` is the unique
        identifier returned by the data‑portal load endpoint.

        Parameters
        ----------
        token : str
            Unique token for the loaded Zarr dataset.

        Returns
        -------
        str
            Base URL of the Zarr store.
        """
        return f"{self.databrowser_url}/zarr/{token}.zarr"

    def zarr_share_store_url(self, token: str, signature: str) -> str:
        """
        Return the base URL for a shared remote Zarr store.

        This is similar to :meth:`zarr_store_url`, but includes a
        signature generated by the ``share_zarr`` endpoint so the store
        ...
        token : str
            Unique token for the loaded Zarr dataset.
        signature : str
            Signature returned by the share endpoint.
