...
def test_load_chunk_keyerror_branch_direct_call(
    ...
    # then load_chunk attempts meta["metadata"][f"{variable}/.zarray"] -> KeyError -> 400
    r = requests.get(
        f"{test_server}/data-portal/zarr/{token}.zarr/this_variable_does_not_exist/.zarray",
        headers={"Authorization": f"Bearer {access}"},
        timeout=10,
    )
    assert r.status_code in (404, 400)
...
def test_aggregate_failed(test_server: str, auth: Dict[str, str]) -> None:
    """Test if we can aggregate data via the rest api."""
    files = requests.get(
        f"{test_server}/databrowser/data-search/freva/file",
        params={"dataset": "agg"},
